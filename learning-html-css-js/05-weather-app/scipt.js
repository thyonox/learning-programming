...
const API_KEY = '6e29a15d3deb4e0e91b192937252304';
...
const weatherCodes = {
    ...
    mist: [1030, 1135, 1147],
    rain: [1063, 1150, 1153, 1168, 1171, 1180, 1183, 1189, 1201, 1240, 1243, 1246, 1273, 1276],
...
const displayHourlyWeather = (hourlyDate) => {
    const currentHour = new Date().setMinutes(0, 0, 0);
    const next24Hours = currentHour + 24 * 60 * 60 * 1000;

    // Filter the hourly data to only include the next 24 hours
    const next24HoursData = hourlyDate.filter(({ time }) => {
    ...
    hourlyWeatherDiv.innerHTML = next24HoursData.map(item => {
        ...
        return `<li class="weather-item">
                <p class="time">${time}</p>
                <img src="assets/${weatherIcon}.svg" class="weather-icon">
...
const getWeatherDetails = async (API_URL) => {
    ...
    window.innerHeight <= 768 && searchInput.blur();
    ...
    try {
        ...
        currentWeatherDiv.querySelector(".weather-icon").src = `./assets/${weatherIcon}.svg`;

        // Combined hourly data from today and tomorrow
        const combinedHourlyDate = [...data.forecast.forecastday[1].hour, ...data.forecast.forecastday[2].hour];
        displayHourlyWeather(combinedHourlyDate);
...
// Set up the weather request for a apecific city
...
locationButton.addEventListener("click", () => {
    navigator.geolocation.getCurrentPosition(position =>{
        ...
        const API_URL = `http://api.weatherapi.com/v1/forecast.json?key=${API_KEY}&q=${latitude},${longitude}&days=3`;
        getWeatherDetails(API_URL);
    }, error => {
